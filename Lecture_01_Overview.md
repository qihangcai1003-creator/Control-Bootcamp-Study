...
1.  **Uncertainty:** Handles model inaccuracies. Real-world systems rarely match our mathematical models perfectly ($A$ and $B$ are imperfect). Feedback corrects for these errors.
2.  **Instability:** Stabilizes naturally unstable systems (e.g., balancing an inverted pendulum or a fighter jet).
3.  **Disturbances:** Rejects external noise or forces ($d$) that push the system off track (e.g., wind gusts on a drone).
4.  **Efficiency:** Achieves performance goals with optimal energy usage, rather than applying maximum force blindly.
